...
MATCH_SYSTEM = (
    ...
    "2. UNKNOWN IS NOT A FAILURE -- this is the most important rule. A criterion "
    "goes in 'not_met' ONLY when the summary states something that EXPLICITLY "
    "CONTRADICTS it. If the summary is simply silent, or the specific detail "
    "(exact severity, a lab value, a confirmatory test) is not given, that is "
    "'unknown', never 'not_met'. Do not manufacture a failure from missing info.\n"
    "3. INCLUSION criteria: list under 'met' if the summary states OR clearly "
    "clinically implies it -- a described textbook presentation of the study's "
    "target condition counts as having that condition (do not demand explicit "
    "lab/keyword confirmation). Put it under 'not_met' ONLY on an explicit "
    "contradiction; otherwise 'unknown'. Note: having the study's OWN condition "
    "is what qualifies a patient -- never treat it as disqualifying.\n"
    "4. EXCLUSION criteria: list under 'not_met' ONLY if the summary EXPLICITLY "
    "shows the patient TRIGGERS the exclusion. If the exclusion is not mentioned "
    "at all, the patient PASSES it -- do NOT list it anywhere and do NOT ask "
    "about every one. Never list an unmentioned or merely-unconfirmed exclusion "
    "as a failure.\n"
    "5. verdict: 'unlikely' ONLY when the patient EXPLICITLY fails a hard "
    "inclusion or EXPLICITLY triggers a hard exclusion (a real contradiction in "
    "the summary). If the only gaps are UNKNOWNS (missing details, unconfirmed "
    "severity/labs), the verdict is 'possible', NOT 'unlikely'. 'likely_eligible' "
    "when the core inclusions are met or clearly implied, no exclusion is "
    "triggered, and few unknowns remain.\n"
    "6. SCORING: use the FULL 0-100 range and DISCRIMINATE between trials. Do "
