...
_READS = [
    ...
    Tool("thread_summary", "read",
         "Summarize the open applicant's whole conversation into a team note.",
         needs_lead=True,
         trace=["Read this applicant's full thread",
                "Drafted a note for your review"],
         run=lambda uid, lid, p: tools.thread_summary(uid, lid)),
    Tool("away_recap", "read",
         "What changed on your conversations while you were away.",
         trace=["Compared your queue against your last away period"],
         run=lambda uid, lid, p: tools.away_recap(uid)),
...
_ACTIONS = [
    ...
    Tool("blast", "action",
         "Message a filtered group of applicants inside ONE study - by stage, "
         "by tag, or everyone who has gone quiet.",
         params={"nct": "the study's NCT id",
                 "stage": "pipeline stage: prescreen | eligible | screening | enrolled",
                 "tag": "conversation tag: consent | docs | followup | pinned",
                 "idle_days": "only people with no activity for this many days"},
         propose="blast",
         trace=["Resolved who matches that description in that study",
                "Drafted a message for your review"]),
    Tool("handoff_coverage", "action",
         "Hand your open conversations to a teammate while you're away.",
         params={"cover": "the teammate who will cover"},
         propose="handoff_coverage",
         trace=["Counted your open conversations",
                "Prepared a handoff for your review"]),
